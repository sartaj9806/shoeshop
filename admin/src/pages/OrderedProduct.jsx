...
const OrderedProduct = () => {
  ...
  const { orderedProduct, backendURL, getAllOrderedProduct, setOrderedProduct, token } = useContext(AdminContext)

  const handleStatusUpdate = async (e, orderId) => {
    const newStatus = e.target.value;

    try {
      const { data } = await axios.put(`${backendURL}/api/order/update-status`, {
        orderId,
        status: newStatus
      }, {
        headers: {
      ...
      if (data.success) {
        // update local state
        setOrderedProduct(prevOrders =>
          prevOrders.map(order =>
            order._id === orderId ? { ...order, status: newStatus } : order
          )
        );
      } else {
        toast.error(data.message + ' OrderedProduct');
      }
    } catch (error) {
      console.error(error);
